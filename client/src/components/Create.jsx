...
async function fetchData() {
	return { projects: await api.get("/projects/lead"), assignees: await api.get("/projects/assignees") }
}
...
const Create = ({ user, setFormOpen, issueCreated, setNotification }) => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			issueCreated({ ...response.data, project: { _id: response.data.project, title } });
			...
			{user && projects ?
				<form onSubmit={handleSubmit}>
					...
					<div className="create__scrollable">
						...
						<label>Issue Type<span className="text-danger">*</span></label>
						<select
						...
						<label htmlFor="issueSummary">Summary<span className="text-danger">*</span></label>
						<input
					...
					<div className="text-end mt-2">
						<input onChange={() => setCreateAnother(!createAnother)} id="create-another" type="checkbox" />
						<label className="me-2" htmlFor="create-another">Create another</label>
						<button disabled={formSubmitting} className="btn btn-primary me-2" type="submit">Create</button>
						<button className="btn btn-link" type="button" onClick={() => setFormOpen(false)}>
							Cancel
						</button>
					</div>
				</form> : <div className="lds-ring"><div></div><div></div><div></div><div></div></div>}
		</div>
	);
};
...
export default connect(mapStateToProps, { issueCreated, setNotification })(Create);
