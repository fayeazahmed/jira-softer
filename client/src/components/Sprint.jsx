...
const Sprint = ({ issues, sprint, index, completeSprint, isLead }) => {
	...
	if (sprintStarted) return <Redirect push to={`/projects/boards/${project}`} />
	...
	return (
		<div className={`sprint ${expanded ? "sprint--expanded" : ""}`}>
			...
			>
				...
				{
					isLead && <div className="d-flex align-items-center">
						<button
							onClick={(e) => {
								e.stopPropagation();
								index === 0 && (isActive ? setCompleteModal(true) : setStartModal(true));
							}}
							className={`me-2 btn ${index === 0 ? (!isActive ? "btn-primary" : "btn-secondary") : "disabled-btn"}`}
						>
			...
			<div tabIndex="0" className="sprint__create">
				{isLead && (createIssue ? <SprintIssueForm setCreateIssue={setCreateIssue} sprint={sprint} forBacklog={false} issueType="story" /> : <h6 onClick={() => setCreateIssue(true)}>+ Create issue</h6>)}
			</div>
		</div>
	);
};
