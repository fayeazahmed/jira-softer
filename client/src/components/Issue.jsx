...
const Issue = ({ match, user, setNotification }) => {
	...
	useEffect(() => {
		function handleKeydown(e) {
			if (e.key?.toUpperCase() === "M" && document.activeElement !== commentRef.current) {
				e.preventDefault()
	...
	const submitComment = async (e) => {
		...
		try {
			...
			setComments([...comments, { ...data, user: { _id: user._id, fullName: user.fullName } }])
		} catch (error) {
			console.log(error.response);
		} finally {
	...
	const getEpicContext = () => <div className="issue__context issue__context--epic">
					...
					<div className="issue__hero">
						<div>
							...
							<div className="issue__comments">
								<h6>Comments</h6>
								{canComment && <form onSubmit={submitComment}>
									<textarea disabled={commentSubmitting} ref={commentRef} value={commentText} onChange={e => setCommentText(e.target.value)} onClick={() => setCommentEditing(true)} className={`${!commentEditing ? "textarea-inactive" : ""}`} placeholder="Add a comment..." type="text"></textarea>
									<div className="text-start">{commentEditing ? <button disabled={commentSubmitting} type="submit" className="btn btn-primary">Comment</button> : <p className="protip"><strong>Pro tip:</strong> press M to comment</p>}</div>
								</form>}
								{
									comments.length > 0 && comments.map(c => <Comment activeComment={activeComment} setActiveComment={setActiveComment} commentDeleteModal={commentDeleteModal} setCommentDeleteModal={setCommentDeleteModal} deleteComment={deleteComment} key={c._id} comment={c} />)
								}
							</div>
						</div>
						<div className="ms-4">
