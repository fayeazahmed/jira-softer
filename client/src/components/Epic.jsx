    ...
    return (
        <div className='epic'>
            <div className="epic__header">
                <i onClick={() => setExpanded(!expanded)} className={`fa fa-2x me-2 fa-angle-${expanded ? "down" : "right"}`} aria-hidden="true"></i>
                <Link to={`/issues/${_id}`}>
                    <Icon size={28} icon="epic" />
                    <div className="progress-container ms-2">
                        <h3>{summary}</h3>
                        {
                            getDoneIssues() > 0 && <div className="progress-bar">
                                <div style={{ width: `${getDoneIssues()}%` }} className="progress-bar__inner"></div>
                            </div>
                        }
                    </div>
                </Link>
                {
                    isLead && <div className="epic__btn">
                        <button onClick={() => {
                            setExpanded(true)
                            setFocus(!focus)
                            setCreateIssue(true)
            ...
            <div className="epic__issues">
                {
                    expanded && issues?.map(i => <Link to={`/issues/${i._id}`} key={i._id}>
                ...
                {
                    expanded && <div tabIndex="0" ref={formRef} className="sprint__create">
                        {isLead && createIssue && <SprintIssueForm setCreateIssue={setCreateIssue} epic={_id} forBacklog={project} issueType="Task" />}
                    </div>
                }
            </div>
        </div>
