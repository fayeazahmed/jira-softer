...
const Backlog = ({ match, issueCreatedDone, created, user, setNotification }) => {
	...
	useEffect(() => {
		if (user)
			(async function () {
				...
				if (data.project.lead._id === user._id)
					setIsLead(true);
			})();
	...
	const completeSprint = async sid => {
		...
		setIssues(issues.filter(issue => issue.sprint !== sid || issue.issueStatus !== "done").map(i => {
...
export default connect(mapStateToProps, { issueCreatedDone, setNotification })(Backlog);
