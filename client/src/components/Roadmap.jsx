...
async function fetchData(id) {
    return { project: await api.get(`/projects/${id}`), issues: await api.get(`/issues/roadmap/${id}`) };
}
...
const Roadmap = ({ match, created, user, issueCreatedDone }) => {
    ...
    useEffect(() => {
        if (user)
            (async function () {
                ...
                if (response.project.data.lead._id === user._id)
                    setIsLead(true);
    ...
    useEffect(() => {
        if (created)
            if (created.project._id === match.params.id && created.issueType === "Epic") {
            ...
            <div className="roadmap">
                ...
                <div className="roadmap__container">
                    ...
                    <div className="roadmap__issues">
                        {issues.map(i => <Epic isLead={isLead} key={i._id} epic={i} />)}
                    </div>
                    <div tabIndex="0" className="sprint__create">
                        {isLead && (createIssue ? <SprintIssueForm setCreateIssue={setCreateIssue} forBacklog={project._id} issueType="Epic" /> : <h6 onClick={() => setCreateIssue(true)}>+ Create Epic</h6>)}
                    </div>
                </div>
            </div>
        </div>
